...
CSock::CSock() {
	...
	if(iSockDllRefCnt++ == 0) {
		...
		if(0 != WSAStartup(MAKEWORD(2,2), &wsaData)) {
			LOGOUT(_T("#ERR %s(%d): 소켓 DLL 초기화 실패. WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
...
CSock::~CSock() {
	try {
		...
		LOGOUT(_T("#ERR %s(%d): ~CSock() 실행 중 예외발생"), _T(__FILE__), __LINE__);
	}
}
...
BOOL CSock::CreateTcpSocket() {
	...
	if(hSock == INVALID_SOCKET) {
		LOGOUT(_T("#ERR %s(%d): WSASocket() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
		return FALSE;
	}
	return TRUE;
}
...
BOOL CSock::Shutdown(int iFlag) {
	...
	*/
	...
	if(SOCKET_ERROR == shutdown(hSock, iFlag)) {
		LOGOUT(_T("#ERR %s(%d): shutdown(iFlag=%d) 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, iFlag, WSAGetLastError());
		return FALSE;
	}
	return TRUE;
}
...
BOOL CSock::Bind(USHORT usPort) {
	...
	if(SOCKET_ERROR == bind(hSock, (SOCKADDR*)&addr, sizeof(SOCKADDR))) {
		LOGOUT(_T("#ERR %s(%d): bind() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
		return FALSE;
	}
	return TRUE;
}
...
BOOL CSock::Listen(IN int iBackLog) {
	if(SOCKET_ERROR == listen(hSock, iBackLog)) {
		LOGOUT(_T("#ERR %s(%d): listen() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
		return FALSE;
	}
	return TRUE;
}
...
BOOL CSock::Accept(OUT CSock* pClientSock) {
	...
	if(pClientSock->hSock == INVALID_SOCKET) {
		LOGOUT(_T("#ERR %s(%d): WSAAccept() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
		return FALSE;
	}
	return TRUE;
}
...
BOOL CSock::Connect(IN ULONG ulIP, IN USHORT usPort) {
	...
	if(SOCKET_ERROR == connect(hSock, (SOCKADDR*)&addr, sizeof(SOCKADDR))) {
		LOGOUT(_T("#ERR %s(%d): connect() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
		return FALSE;
	}
	return TRUE;
}
...
int CSock::Recv(OUT char* pRecvBuf, IN int iBufLen) {
	...
	if(iRecvedBytes == SOCKET_ERROR) {
		LOGOUT(_T("#ERR %s(%d): recv() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
		return SOCKET_ERROR;
	}
	return iRecvedBytes;
}
...
int CSock::Send(IN const char* pSendBuf, IN int iSendDataLen) {
	...
	while(iTotalSentBytes < iSendDataLen) {
		...
		if(iSentBytes == SOCKET_ERROR) {
			LOGOUT(_T("#ERR %s(%d): send() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
			return SOCKET_ERROR;
		}
...
int CSock::RecvFrom(OUT char* pRecvBuf, IN int iBufLen, OUT SOCKADDR_IN& addr) {
	...
	if(iRecvedBytes == SOCKET_ERROR) {
		LOGOUT(_T("#ERR %s(%d): recvfrom() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
		return SOCKET_ERROR;
	}
	return iRecvedBytes;
}
...
int CSock::SendTo(IN char* pSendBuf, IN int iSendDataLen, IN SOCKADDR_IN& addr) {
	...
	if(iSentBytes == SOCKET_ERROR) {
		LOGOUT(_T("#ERR %s(%d): sendto() 실패, WSAGetLastError=%d"), _T(__FILE__), __LINE__, WSAGetLastError());
		return SOCKET_ERROR;
	}
	return iSentBytes;
}
