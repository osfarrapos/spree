	...
	while( bContineServer ) 
	{
		...
		case WAIT_OBJECT_0 + 1:	// 대기중인 Accept 가 없다. 이거 들어오면 무조건 10번씩 호출하자.
			...
			*/
			...
			/*
#ifdef _TRACE_
			g_pTrace->OutputString( _TRACE_CLASS_SYSTEM, "****  NoAcceptWait Event : Accept %d  , cur:%d, min : %d, max:%d \n", temp, m_nCurPendingAccept, m_nMinPendingAccept, m_nMaxPendingAccept ) ;
#endif
			*/
...
/*
int CServerSocket::WorkerThread::Run()
{
	DWORD dwReceived =0 ;
	SOCKET s = 0 ;
	COV_UNIT * pUnit = NULL ;
	...
	try 
	{
		while( true )
		{
			...
			try {
				m_iocp.GetStatus( (PDWORD)&s, &dwReceived, (LPOVERLAPPED*)&pUnit ) ;
			}
			catch ( ... ) {
				printf( "[EXCEPTION] GetStatus\n" ) ;
				throw ;
			}
			...
			case mode_recv:
				...
				{
					m_server.OnReceive( static_cast<CSocketBuffer*>(pUnit), dwReceived ) ;
				}
				break ;
