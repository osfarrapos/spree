...
public:
	CSqlConnectorString( char * fileName ) : _j_Crypto(4096)
	{
		...
		if( Init( fileName ) == false ) 
		{
			printf( " -Error- Crypto file read.\n" ) ;
	...
	{
		if( fileName == NULL || m_bInitSuccess == false )
		{
			return false ;
		}
		...
		if(fclose(stream) != 0) 
		{
			printf( "data.enc - Read Fail!\n") ;
			return false ;		
		}

		if( Decryption( buf, sizeof(buf), 'N') == false )
		{
			printf( "data.enc Decryption Error \n") ;
			return false ;
		}
