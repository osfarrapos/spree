...
bool CFileHeader::Read(CZipStorage *pStorage)
{
	...
	pStorage->m_file.Read(filename, m_uFileNameSize);

	TCHAR widestring[1024];
	memset( widestring, 0, sizeof(TCHAR) * 1024 );
	int requirelength = MultiByteToWideChar(CP_ACP, 0, filename, strlen(filename), widestring, 1024);

	//m_szFileName.GetBuffer(m_uFileNameSize)
	//m_szFileName.ReleaseBuffer(m_uFileNameSize);
	m_szFileName = widestring;
