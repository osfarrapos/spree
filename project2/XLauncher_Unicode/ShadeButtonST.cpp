...
void CShadeButtonST::SetShade(UINT shadeID, BYTE granularity, BYTE highlight, BYTE coloring, COLORREF color)
{
	...
	for(i=0;i<129;i++){
		r=((128-i)*GetRValue(locr)+i*GetRValue(midcr))/200;
		g=((128-i)*GetGValue(locr)+i*GetGValue(midcr))/200;
		b=((128-i)*GetBValue(locr)+i*GetBValue(midcr))/200;
	...
	for(i=1;i<129;i++){
		r=((128-i)*GetRValue(midcr)+i*GetRValue(hicr))/200;
		g=((128-i)*GetGValue(midcr)+i*GetGValue(hicr))/200;
		b=((128-i)*GetBValue(midcr)+i*GetBValue(hicr))/200;
