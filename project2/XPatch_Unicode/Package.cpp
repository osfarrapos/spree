...
BOOL CPackage::Open( LPCTSTR filename, BOOL skipcreateindextable )
{
	...
	_XLog( _T("Package Message : Package opened [%s]\n"), indexfilename );
...
{	
	...
	_XLog( _T("Package Message : Export file [%d:%s]\n"), file_id, filename );
	...
	_XLog( _T("Package Message : Check folder and create [%d:%s]\n"), file_id, filename );
	...
	_XLog( _T("Package Message : Exporting [%d:%s]\n"), file_id, filename );
	...
	_XLog( _T("Package Message : Check old file [%d:%s]\n"), file_id, filename );
	...
	_XLog( _T("Package Message : Rename new file [%s->%s]\n"), newfilename, filename );
...
BOOL CPackage::ExportFile( int file_id, FILE* pExportFile )
{
	...
	_XLog( _T("Package Message : Search Export file [%d]\n"), file_id );
	...
	_XLog( _T("Package Message : Exporting [%d]\n"), file_id );
	...
	_XLog( _T("Package Message : Read org file [%d]\n"), file_id );
	...
	_XLog( _T("Package Message : write new file [%d]\n"), file_id );
...
BOOL CPackage::Patch( LPCTSTR packagefilename, LPCTSTR patchinfofilename )
{
	...
	if( !pPatchinfoFile )
	{
		AfxMessageBox( _T("ERROR : Upgrade information file open") );
		this->Close();
		return FALSE;
	}

	_XLog( _T("Package Message : Open patch information file [%s:%s]\n"), packagefilename, patchinfofilename );
	...
	//_XRegReadString( ID_REGSTR_MASTERKEY, _T("Software\\Indy21\\NineDragons\\"), _T("Path"), _T(""), registrybuffer, _REGSTRING_MAXLENGTH, FALSE);
	...
	for( int packageindexcount = 0; packageindexcount < patchfilecount; packageindexcount++ )
	{
		...
		{				
			...
			tMessagestring.Format( _T("Upgrade complete.[%s]\n"), szPatchFileString[packageindexcount] );
...
BOOL CPackage::PatchXPToXP( LPCTSTR packagefilename )
{
	...
	tMessagestring.Format( _T("Calculate to package informations.\n") );
	//((CXLauncherDlg*)AfxGetMainWnd())->InsertStringToProgressMessageListCtrl(tMessagestring);
	_XLog( tMessagestring.GetBuffer(256) );
