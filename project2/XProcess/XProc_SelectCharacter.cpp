...
//#define _XDEF_TESTRENDER

#ifdef _XDEF_TESTRENDER	// 미국 XML 번역툴용에 들어갈 이미지 추출용 플래그.
...
bool XProc_SelectCharacter::InitDeviceObject(void)
{
	...
	if( pSelectCharacter_Window )
	{
		...
		for( int i = 0; i < g_pLocalUser->m_CharacterCount; i++ )
		{
			_XLog( "%d, %d, %s", i, g_pLocalUser->m_AllCharacterInfo[i].zoneuniqid-1, g_ZoneInfoTable[g_pLocalUser->m_AllCharacterInfo[i].zoneuniqid-1].zonefilename );
		}
		*/
...
void _XTRCaptureScreen( void )
{
	...
	for( int xx = 1730 ; xx < _XDEF_MODELCLASSCOUNT_MOB; xx++ )
	{
		if(!g_MOBMODEL[xx])
			continue;
